    ...
    <div class='d-flex flex-column justify-content-center container mt-5 mb-5 '>
        <form method='POST' class='d-flex justify-content-center align-center flex-column'>
            ...
            <select name='validaAdm' id='validaAdm' onchange='window.location="/registrar/" + this.value'
                ...
                <option @if($id == 1) {{'selected'}} @endif value="1">PERFIL ADMINISTRADOR{{$id}}</option>
                <option @if($id == 0) {{'selected'}} @endif value="0">PERFIL FUNCIONÁRIO</option>
            </select>
            <input name='cod_func' type='number' class='neomorphic-input margin-05 padding-1 borderRadius-20 col-4 validate' title='Código ou matrícula do funcionário' placeholder='Código Funcionário' required>
            <input name='adm_id' id='adm_id' type='number' class='neomorphic-input margin-05 padding-1 borderRadius-20 col-4' title='Código ou matrícula do administrador.' placeholder='Código do Administrador' required>
...
<div id='mobile'>
    ...
    <div class='d-flex flex-column justify-content-center container mt-5 mb-5 '>
        <form method='POST' class='d-flex justify-content-center align-center row'>
            ...
            <input name='cod_func' type='number' class='neomorphic-input margin-05 padding-1 borderRadius-10 col-12 validate' title='Somente números' placeholder='Código Funcionário' required>
            <input name='adm_id' id='adm_id' type='number' class='neomorphic-input margin-05 padding-1 borderRadius-10 col-12' title='Código ou matrícula do administrador.' placeholder='Código do Administrador' required>
