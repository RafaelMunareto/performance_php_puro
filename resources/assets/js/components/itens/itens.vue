        ...
        <div v-else>
            ...
            <div class='ml-1'>
                <div class='row'>
                    ...
                    <a href='#' @click="sort($event, 'categoria')" class='neomorphic-table-thead col-1'><div><span>CATEGORIA</span></div></a>
                ...
                <div v-for="u of orderBy(buscaFiltro, sortProperty, sortDirection)" :key="u.id"  class='row'>
                    ...
                    <div class='neomorphic-table col-1'><span class='size80'>{{ u.categoria }}</span></div>
                    ...
                    <div class='neomorphic-table col-1  d-flex justify-content-center'><a :href="`/editar/${u.id}`"><i class="fas fa-eraser fa-1x text-green size125"></i></a></div>
...
export default {
    ...
    computed:{
        ...
        buscaFiltro() {
            ...
            if (this.filtro) {
                ...
                this.visivel = false
                return this.itens.filter(adm => exp.test([adm.cod_item,adm.nome_item,adm.categoria,adm.obj,adm.peso ]));
            } else {
                this.visivel = true
    ...
    methods:{
        ...
        ...mapActions('item', ['ActionItens', 'ActionItensPag', 'ActionItensDelete']),
       ...
       async atualizaBase(date){
        ...
        async atualizaBasePag(date,paginate,page){
            try{
                await this.ActionItensPag({date,paginate,page})
            }catch (err){
                console.log('Não foi possível carregar a página')
            }finally{
                this.loading = false
